  ...
  <v-card v-if="dataUpToDate">
    <v-card-title primary-title>
      <div>
        ...
        <div class="headline text-uppercase" v-model="total">{{total}}</div>
      </div>
    </v-card-title>
    ...
    <v-card-actions>
      ...
      <v-layout align-center justify-center column fill-height>
        ...
        <v-flex xs12>
          <span class="text-uppercase headline" v-model="yesterdayCategoryTotal">{{yesterdayCategoryTotal}}</span>
        </v-flex>
      ...
      <v-layout align-center justify-center column fill-height>
        ...
        <v-flex xs12>
          <span class="text-uppercase headline" v-model="lastWeekCategoryTotal">{{lastWeekCategoryTotal}}</span>
        </v-flex>
        <v-layout align-center justify-center row fill-height>
      ...
      <v-layout align-center justify-center column fill-height>
        ...
        <v-flex xs12>
          <span class="text-uppercase headline" v-model="lastMonthCategoryTotal">{{lastMonthCategoryTotal}}</span>
        </v-flex>
        <v-layout align-center justify-center row fill-height>
      ...
      loadChartData(tweets) {
        if(tweets != null && tweets.length > 1){
          ...
          tweets.forEach((tweet, index) => {
            let tweetIsCategoryCount = tweet.tweet_class === self.tweetCategory ? 1 : 0
